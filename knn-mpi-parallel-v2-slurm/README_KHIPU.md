# Khipu and Slurm Workflow for KNN-MPI v2
...
rsync -av --exclude ".git" --exclude ".venv" --exclude "results" \
  knn-mpi-parallel-v2-slurm/ USER@KHIPU_HOST:~/knn-mpi-parallel-v2-slurm/
```
...
rsync -av wheelhouse/ USER@KHIPU_HOST:~/knn-mpi-parallel-v2-slurm/wheelhouse/
```
...
```bash
squeue -u "$USER"
tail -f logs/knn_mpi_v2_bench_JOBID.out
tail -f logs/knn_mpi_v2_bench_JOBID.err
scancel JOBID
```
...
```bash
rsync -av USER@KHIPU_HOST:~/knn-mpi-parallel-v2-slurm/results/ ./khipu_v2_results/
rsync -av USER@KHIPU_HOST:~/knn-mpi-parallel-v2-slurm/logs/ ./khipu_v2_logs/
```
