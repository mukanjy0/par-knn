#!/usr/bin/env bash
# Run a reproducible KNN-MPI v2 experiment matrix and keep raw rows in one CSV.
set -euo pipefail
...
OUT_DIR="${OUT_DIR:-results/v2_${STAMP}}"
...
echo "============================================================"
echo "KNN-MPI v2 experiment matrix"
echo "Date:       $(date)"
...
for n in $SAMPLES; do
  ...
  for p in $PROCS; do
    ...
    "$MPI_LAUNCHER" $MPI_EXTRA_ARGS -n "$p" $PYTHON src/knn_mpi_v2.py \
