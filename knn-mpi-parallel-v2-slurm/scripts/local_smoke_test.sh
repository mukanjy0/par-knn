#!/usr/bin/env bash
# Tiny local validation for v2. This does not install MPI and does not run heavy jobs.
set -euo pipefail
...
PROCS="${PROCS:-1 2 4}"
OUT_DIR="${OUT_DIR:-results/local_smoke_v2_${STAMP}}"
...
if [ -n "$MPI_LAUNCHER" ]; then
  ...
  for p in $PROCS; do
    echo "-- p=$p"
    "$MPI_LAUNCHER" -n "$p" $PYTHON src/knn_mpi_v2.py \
