# Khipu and Slurm Workflow for KNN-MPI v3
...
rsync -av --exclude ".git" --exclude ".venv" --exclude "results" \
  knn-mpi-parallel-v3-slurm/ USER@KHIPU_HOST:~/knn-mpi-parallel-v3-slurm/
```
...
rsync -av wheelhouse/ USER@KHIPU_HOST:~/knn-mpi-parallel-v3-slurm/wheelhouse/
```
...
The timed MPI code keeps the v3 collective structure:
...
```bash
squeue -u "$USER"
tail -f logs/knn_mpi_v3_bench_JOBID.out
tail -f logs/knn_mpi_v3_bench_JOBID.err
scancel JOBID
```
...
```bash
rsync -av USER@KHIPU_HOST:~/knn-mpi-parallel-v3-slurm/results/ ./khipu_v3_results/
rsync -av USER@KHIPU_HOST:~/knn-mpi-parallel-v3-slurm/logs/ ./khipu_v3_logs/
```
