#!/usr/bin/env bash
# Run a reproducible KNN experiment matrix and keep raw rows in one CSV.
set -euo pipefail
...
OUT_DIR="${OUT_DIR:-results/knn_${STAMP}}"
...
echo "============================================================"
echo "KNN-MPI experiment matrix"
echo "Date:       $(date)"
...
for n in $SAMPLES; do
  ...
  for p in $PROCS; do
    ...
    "$MPI_LAUNCHER" $MPI_EXTRA_ARGS -n "$p" $PYTHON src/knn_mpi_v3.py \
